            ...
            <div class="content-card space-y-4">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-slate-100 pb-3">
                    ...
                    <form method="GET" action="{{ route('volunteer.census.index') }}" class="flex flex-wrap items-center gap-2">
                        ...
                        <div class="relative">
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari ID / Zona / Warna..." class="form-input text-xs py-1.5 pl-3 pr-10 min-h-[38px] w-48 sm:w-60">
                            <div class="absolute right-2.5 top-1/2 -translate-y-1/2 flex items-center gap-1">
                                @if(request('search'))
                                    <a href="{{ route('volunteer.census.index', ['kampus' => request('kampus')]) }}" class="text-slate-400 hover:text-slate-600 text-xs font-bold" title="Hapus pencarian">✕</a>
                                @endif
